//
//  RangeReplaceableCollection+2d.swift
//  AdventOfCode
//
...
extension RangeReplaceableCollection where Self: MutableCollection,
  ...
  mutating func grow(repeating defaultValue: Element.Element, by count: Int = 1) {
    // Validate count.
    guard count > 0 else {
      if count < 0 {
    ...
    guard !isEmpty else {
      self = Self(repeating: Element(repeating: defaultValue, count: count), count: count)
      return
    }
    ...
    let newElement = Element(repeating: defaultValue, count: first!.count)
