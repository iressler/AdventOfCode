...
extension Collection where Element: Collection, Element.Index == Index, Element.Element: Collection, Element.Element.Index == Index, Index == Int {
...
extension MutableCollection where Element: MutableCollection, Element.Index == Index, Element.Element: MutableCollection, Element.Element.Index == Index, Index == Int {
  subscript(point: Point) -> Element.Element.Element  {
    get {
      return self[point.z][point.x][point.y]
    }
  ...
  func pointsAdjacent(to point: Point, includeDiagonals: Bool = false) -> [Point] {
    ...
    if point.z > 0 {
      let newPoint = Point(x: point.x, y: point.y, z: point.z-1)
